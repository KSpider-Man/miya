	...
	$.ajax({
		...
		success:function(rel){
			...
			for(let i=1;i<rel.data.length;i++){
				...
				var $img = `<img id=${rel.data[i].id} class="shopImg" src=${rel.data[i].picurl}>`;//商品图片
				var $a = `<a href="#">${rel.data[i].name}</a>`;//商品名字
				var $span = `<span>￥${rel.data[i].price}</span>`;//价格	
				var $submit =`<input type="button"  class="submit" value="加入购物车">`;//准备转入详情页的按钮
				$figure.append($img);
				$figure.append($a);
				$figure.append($span);
			...
			for(let j=0;j<$(".shopImg").length;j++){//每个按钮添加事件
				$(".shopImg").eq(j).on("click",function(){
					var aid=$(".shopImg").eq(j).attr("id");
					console.log(aid);
					window.location.href = 'http://127.0.0.1:8848/miya/fangdajing.html?aid='+aid;//我这个改了
				});
			}
		}
	});
